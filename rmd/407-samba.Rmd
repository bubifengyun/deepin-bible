...
但是，对于局域网的Deepin深度用户来讲，除了[iptux信使](https://github.com/iptux-src/iptux)（只支持Linux、Mac，号称GNU/Linux版飞鸽传书）、[飞鸽传书](http://www.feige360.com/)（最新版跨平台支持Linux）外，还可以安装samba 文件传输服务器。
...
![共享文件夹出现的问题](samba-share.png)

由于权限的问题，取消共享也会出现类似问题，也请采用管理员权限或者修改配置文件的方法取消共享。

![取消共享出现的问题](samba-unshare.png)
...
![共享文件夹出现的问题\label{samba-hotwif}](samba-hotwifi.png)