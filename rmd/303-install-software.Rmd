...
除了直接下载源码之外，还有个很好的源码安装管理工具`Homebrew`，原本是Mac OS X 平台下 Ruby 开发的包管理软件，现在也支持[Linux平台](https://github.com/Homebrew/linuxbrew-core)了。这个包管理软件在Linux也不是非常成熟。仅作了解。
...
这里以 `you-get`（？？？pip吧） 软件为例，介绍 npm 软件的安装方式。
...
这个参考[官网](https://developer.android.google.cn/studio/install)就可以了。其他不说了。
...
下面以texlive的安装为例，首先到国内镜像下载texlive的.iso文件，本人的[博客](https://my.oschina.net/bubifengyun/blog/1936119)记录了texlive2018的安装方式。安装方式比较傻瓜，相比于windows下一个个下一步按键来说，这里需要对着终端做好选择，然后安装。
