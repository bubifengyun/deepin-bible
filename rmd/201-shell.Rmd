...
|       管道            |            重定向             |
+======================+==============================+
| 左边命令应该做到标准输出 |   左边的命令应该有标准输出(输入)  |
| 右边命令应该接受标准输入 |     >（<） 右边只能是文件       |
+----------------------+------------------------------+
| 管道触发两个子进程执行   |        在一个进程内执行        |
| `|`两边的程序          |                              |
+----------------------+------------------------------+
...
BASH This contains the full pathname of the bash command. This is usually /bin/
bash.
...
the shell starts, based on the user’s entry in the /etc/passwd fle.
...
HISTFILE This is the location of your history fle. It is typically located at $HOME/.
bash_history.
...
x86_64.
MAIL This is the location of your mailbox fle. The fle is typically your username in
the /var/spool/mail directory.
OLDPWD This is the directory that was the working directory before you changed to
the current working directory.
OSTYPE This name identifes the current operating system. For Fedora Linux, the
...
PATH This is the colon-separated list of directories used to fnd commands that
you type. The default value for regular users varies for different distributions, but typically includes the following: /bin:/usr/bin:/usr/local/
...
就是在前面\@ref(shell:intro)提到的`$`或者`#`之前的文字。是通过环境变量`PS1`，`PS2`，`PS,3`，`PS4`配置的。
